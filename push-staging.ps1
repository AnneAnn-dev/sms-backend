...
if (-not (Test-Path $refFile)) {
    Write-Host "❌ Ingen link fundet ($refFile mangler). Koer foerst:" -ForegroundColor Red
...
if ($ref -ne $STAGING_REF) {
    Write-Host "❌ STOP: CLI'en er linket til '$ref' — IKKE staging ($STAGING_REF)." -ForegroundColor Red
    Write-Host "   Skift link foerst:  npx supabase link --project-ref $STAGING_REF"
    exit 1
}

Write-Host "✅ Link verificeret: staging ($ref) — pusher migrationer..." -ForegroundColor Green
