...
**Ejer:** Ann
**Oprettet:** 2026-08-01
**Sidst gennemgået:** 2026-08-09 — første kundeafvikling gennemført · D30, D31 og S15 oprettet · J4 opdateret med fund fra afviklingen · nummerlageret nede på 1 ledigt
...
| 15 | `Email OTP expiration` 86.400 → 3.600 sek. i begge projekter | S3 | 10 min | ⬜ **Åben — NU.** Ét felt. Kræver en kort snak med Anne om velkomstmailens link, se S3 |
...
| S14 | **Adgangskodepolitikken er svagere end den, produktet lover — og to gratis værn står slukket.** Aflæst i BEGGE projekter 8/8: `Minimum password length` = **6**, mens onboardingens tekst siger *"Minimum 8 tegn."* — UI'et lover mere, end serveren håndhæver, og serveren er den, der gælder. `Password requirements` står på ingen krav (ingen tegnklasser). `Prevent use of leaked passwords` er **slået fra**, selv om det er inkluderet i Pro-planen og kun er en kontakt: det afviser adgangskoder, der er kendt lækkede. Sammenholdt med sign-in-loftet på 360 forsøg/t pr. IP er seks tegn uden ordbogsværn en reel gættevej — og genbrugte, lækkede adgangskoder er den mest sandsynlige måde, en håndværkerkonto falder på | M | H | Pilot · min | Åben | Tre kontakter og én tekst: **(1)** `Minimum password length` → 8, så serveren siger det samme som onboardingen · **(2)** `Prevent use of leaked passwords` → til (HaveIBeenPwned-opslag, ingen kode) · **(3)** overvej ét tegnklassekrav — men vej det mod målgruppen, for et krav, der presser folk til `Sommer2026!`, køber ikke meget · **(4)** ⚠️ `Require current password when updating` står også slukket, og den **kan ikke bare tændes**: onboardingens trin 1 sætter adgangskoden på en bruger, der er logget ind via magic link og *ingen* nuværende kode har. Tændes den, kan onboardingen brække. Konsekvensen af at lade den stå slukket er, at en kapret session kan skifte adgangskoden uden at kende den gamle — hør sammen med S9. **Afklar, om Supabase behandler "ingen kode sat" som en undtagelse, før der røres ved kontakten.** **Oprettet 8/8 ved aflæsningen af OTP-længden** |
...
| 2026-08-09 | **D30 oprettet, M/L:** `afstem-numre.js` crasher ved exit på Windows → exit-koden er ubrugelig, selv om outputtet er korrekt |
...
| 2026-08-09 | **S15 oprettet, M/M:** intet teknisk stopper et uindløst magic link ved kundeafvikling — kun sletning af Auth-brugeren. Forstærkes af S3's 24-timers gyldighed |
