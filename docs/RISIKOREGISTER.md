...
| 2026-08-05 | **Ø5 trin 1 udført: `require("./frisbii-checkout")(app);` monteret i `server.js`** lige efter `frisbii-webhook`, med kommentar om fejlmåden. Verificeret ved kodegennemgang før redigering: monteringen kan ikke crashe ved boot, fordi `requireConfig()` først kaldes inde i rutehandleren — rækkefølgen montering → variabler holder. **Ø5 lukkes ikke her:** trin 2 (variabler) og 3 (Simply-sider) udestår, og prod kører fortsat på test-nøgler |
