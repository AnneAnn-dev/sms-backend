...
**Roller:** Anne ejer backend, infrastruktur og al kode. [Makkerens navn] ejer produktplanen, det visuelle udtryk, kundevendt indhold og QA/røgtest — hans mockups, tekster og AI-prototyper er *spec*, som Anne implementerer (prototyper går aldrig direkte i kodebasen). Detaljeret samarbejdsmodel: se `lommekontor-drift-runbook.md` → "Partner som bidragyder".
...
- **Backend:** Node.js/Express på Railway (EU West), repo `AnneAnn-dev/sms-backend`, prod-URL `sms-backend-production-5ee1.up.railway.app`
- **Database + Auth:** Supabase (Frankfurt) — projekt `glymuxqtrbpeyzmflilf.supabase.co`. RLS på alle tabeller.
- **Telefoni/SMS:** Twilio (46elks noteret som muligt post-launch-alternativ)
- **E-mail + storage:** Scaleway TEM (HTTP API, ikke SMTP) og Supabase Storage
- **TTS/stemmer:** ElevenLabs (`eleven_multilingual_v2`), pre-renderet og afspillet via `<Play>`-URL. Polly-fallback (Polly.Mads/Polly.Naja) hvis ingen lydfil.
- **Billing:** Frisbii med MobilePay Recurring
- **DNS:** Simply.com
...
- **Overvågning:** AppSignal (EU, Amsterdam) — fejlsporing. Se *Drift & udvikling*.

Scripts køres **lokalt fra PowerShell** i `sms-backend`-mappen (Windows/VS Code), ikke på Railway. `dotenv` indlæser `.env`.
...
- **Dashboard** logger ind med `signInWithPassword`. Login-skærmen har en redningsvej: **"Send mig et login-link"** → `POST /onboarding/nyt-link` (samme prefetch-sikre Scaleway-mail som onboardingens "linket virker ikke"-skærm; anti-enumeration; rate-limited). Linket lander på `/onboarding`, som sender aktive firmaer videre til `/dashboard`.
...
- **Felter:** Navn, telefon, adresse, beskriv opgaven, **Haster**, noter, billeder. Deadline og "Ønsket tidspunkt fra kunde" er **fjernet**. **Haster** er et alm. labelet felt ("HASTER" som overskrift + en klikbar boks med kryds-ikon der skifter til rød ved aktiv) — og persisteres nu faktisk ved luk (`is_urgent` er lagt ind i `closeModal`'s update; før blev kun adresse/mail/noter gemt).
...
- **Ny opgave-modal:** "Hvornår passer det dig?"-feltet er ligeledes fjernet.
...
- **phone_numbers (pulje):** `id`, `number` (E.164), `twilio_sid` (PN…), `firm_id` (**null = ledigt**). Provisionering (frisbii-webhook.js + provision-test-firm.js) claimer udelukkende ved at sætte `firm_id` — ingen `status`-kolonne i pulje-flowet.
...
- Adskillelse af `status` (onboarding-livscyklus) vs. `billing_status` (abonnement). Frisbii-webhooks svarer **straks 200** for at undgå deaktivering.
...
- **buy-numbers.js** — køber DK-mobilnumre, sætter `/opkald`-webhook i samme kald, lægger i puljen. `DRY_RUN=1 node buy-numbers.js 3` viser kandidater gratis; uden DRY_RUN købes for rigtige penge.
- **configure-number.js** — til numre købt manuelt i konsollen: sætter webhook + lægger i pulje (tager PN-SID).
...
- **reset-test-data.js** — fuld nulstilling: sletter alle firmaer + koblinger + auth-brugere i FK-rækkefølge og frigør alle numre (beholder pulje-rækkerne). `--dry-run` viser kun optælling. Se også **opskriftsbogen** (`lommekontor-opskriftsbog-numre.md`) for numre/testdata-oprydning trin for trin.
...
**Railway + lokal:** `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN`, `TWILIO_SYSTEM_NUMBER`, `ELEVENLABS_API_KEY`, `ELEVENLABS_VOICE_IDM`, `ELEVENLABS_VOICE_IDF`, `BASE_URL`, `SCW_SECRET_KEY`, `SCW_PROJECT_ID`, `SCW_REGION`, `FRISBII_WEBHOOK_SECRET`, `APPSIGNAL_PUSH_API_KEY`, `APPSIGNAL_APP_NAME`, `APPSIGNAL_APP_ENV` (sidstnævnte **pr. miljø**: production/staging)
**Kun lokal (til nummer-køb):** `TWILIO_ADDRESS_SID` (AD…)
...
*Detaljeret opskrift i `lommekontor-drift-runbook.md`. Princip: **én kodebase, to verdener** (staging/prod), **fundament før features**.*
...
*Status: AppSignal (trin 0) er på plads (se nedenfor). Resten af fundamentet er ikke bygget endnu.*
