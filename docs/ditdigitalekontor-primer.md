...
Rækkefølge (ændret 22/8, Anns valg): profilkort → **Numre uden SMS** (hvidliste) → **Din telefonbesked** → **Din viderestilling** → **Skift adgangskode** → **Beskeder** (push-kort, se nedenfor) → Log ud.
...
- **Beskeder (nyt 19/8 som del af D34, flyttet til bunds og knappen omdøbt 22/8):** til/fra-knap for push-notifikationer (badge på PWA-ikonet + selve notifikationen), bygget på VAPID web push + service worker + tabellen `push_subscriptions`. Knappen (`#push-toggle-btn`, tekst i `#push-toggle-label`, status i `#push-status`) kalder den delte `skiftPushBeskeder()` (bruger `pushStoettes()`, `hentPushAbonnement()`, `opdaterPushKnap()`), som abonnerer/afmelder mod `/api/push/subscribe` og `/api/push/unsubscribe` — feature-detected og fail-open, så manglende browserstøtte aldrig vælter siden. **Dette er nu den ENESTE vej til at slå notifikationer til/fra** — der bedes ikke længere om tilladelse i app-hilsenen ved første åbning (se App-hilsen nedenfor). Se `RISIKOREGISTER.md` D34 for hele historikken.
- **App-hilsen (16-17/7, i prod):** første gang dashboardet åbnes som installeret app (`display-mode: standalone` / `navigator.standalone`) vises én gang: grønt ✓-mærke + "Dit Digitale Kontor er klar." + [Åbn] (localStorage-flag `ddk-app-hilsen-vist`). Slutningen på onboardingens installations-rejse — den installerede app åbner i egen kontekst, så øjeblikket SKAL fanges her. Bevidst helt selvstændig blok (egen markup + eget script, alt i try/catch): kan aldrig vælte dashboardet, jf. hvid-skærm-historikken. Anne ser den én enkelt gang — ventet, ikke en fejl. ⚠️ **19/8-22/8: bar kortvarigt en ekstra knap her, der bad om notifikations-tilladelse ("Slå beskeder til").** Den brød med princippet om superenkel opstart — at bede om en tilladelse i samme åndedrag som "installer app"-rejsen er en unødig forhindring ved allerførste åbning — og knappen er fjernet igen 22/8, uden at røre selve push-løsningen. Kun [Åbn] er tilbage; notifikationer er nu udelukkende Profil-sidens Beskeder-kort. Se `RISIKOREGISTER.md` D34.
