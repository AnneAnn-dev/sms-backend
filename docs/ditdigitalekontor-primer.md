...
- **s-1 Velkommen:** "Hvilket nummer skal vi passe på?" — er `owner_phone` sat, bekræftes det ([Forbind nummeret] / Skift nummer); ellers indtastning m. fast +45-prefiks (præcis 8 cifre). Gemmer via `/api/firma/opdater-telefon`.
...
- **s-11 Færdig:** tro kopi af hjemmeskærms-ikonet (`/apple-touch-icon.png?v=2`; vises først onload — aldrig et knækket billede) m. label **"Dit Kontor"** (SKAL matche manifestets `short_name`/`apple-mobile-web-app-title` — tjek udestående i runbook) + "Åbn Dit Digitale Kontor fra ikonet." Stille flugtveje: "Kan du ikke finde ikonet? Prøv igen" (→ s-9) og ghost-link til /dashboard.
...
- **Affødte udeståender:** manifest-`short_name` skal matche "Dit Kontor"; evt. preview-endpoint til ▶ Lyt (i dag koster hvert Lyt m. ændret tekst et ElevenLabs-kald).
