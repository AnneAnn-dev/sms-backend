...
- **Frisbii:** kør i **test-/sandbox-tilstand** i staging, så et test-checkout aldrig trækker et rigtigt kort. ✅ *Færdig og røgtestet end-to-end 5/7-26: hosted checkout (testkort 4111...) → `invoice_settled` → staging-webhook (POST 200) → provisionering (firma + nummer claimet + auth-bruger) → `[STAGING -> ...]`-velkomstmail + pulje-alarm modtaget. Prod verificeret uberørt.*
  **Kontomapping** (autoritativ HER — handles er permanente, navne kan ændres; verificér ALTID via handle i browserens URL, ikke via visningsnavnet):
  ...
  11. **⚠️ Tre verifikationsregler, hver købt dyrt 16/8-26.**
     **(a) En LEVERET rettelse er ikke en INDLAGT rettelse.** `company`-kravet blev skrevet, men filen blev aldrig lagt ind. Alt så rigtigt ud: `smoke:prod` grøn 11/11, sessioner blev oprettet, og git var enig med sig selv (`main` = `staging`, tom diff). Kun det kald, der ramte præcis den ene ændring, afslørede det. **"Deployet virker" og "min ændring er med i deployet" er to forskellige påstande.**
     **(b) Verificér med noget, der ikke fandtes før.** `missing_company` er den rigtige prøve, fordi den fejlkode ikke eksisterede i den gamle kode. En 400, der også fandtes før, skelner ingenting — og en 200 beviser kun, at ruten svarer. Samme logik som den negative kontrol i røgtesten.
     **(c) Testartefakter bærer det forrige forsøgs resultat med sig.** `body.json` narrede to gange på én dag: en 200 blev læst som bevis for noget, filen slet ikke testede, fordi den stadig indeholdt et firmanavn fra sidste kørsel. **Skriv filen om i samme åndedrag som kaldet** — aldrig genbrug en body, du ikke lige har set indholdet af.
     📌 **Og vent på deployet:** Railway svarer med den GAMLE container, mens den nye bygger. Et kald under ét minut efter push måler den gamle kode og ligner en fejlet rettelse.
  12. **⚠️ Afstemning mellem to kopier er ikke sandhed (16/8-26).** Prod svarede `404 Subscription plan not found`, mens `afstem-railway-env.js` viste **0 afvigelser**: `.env.prod` og Railway var enige med hinanden om et plan-handle, ingen af dem havde spurgt Frisbii om. **Værktøjet sammenligner filen med Railway — ikke Railway med leverandøren.** Den slags hul lukkes kun af et tjek, der spørger leverandøren (jf. D26).
  13. **Kontonavnene er omdøbt til noget læsbart (16/8-26)** — LIVE *Dit Digitale Kontor* · TEST *Prod Dit Digitale Kontor* · TEST *Test Dit Digitale Kontor* · *oprettelse og abonnement*. ⚠️ **Handlet er fortsat det eneste autoritative:** navne kan ændres, handles er permanente. Skal det afgøres, hvilken konto en nøgle hører til, er den empiriske prøve at finde en session, koden lige har oprettet — eller `check-env.js --live`, som spørger Frisbii direkte.
...
3. ✅/⛔ **Checkout virker i STAGING hele vejen (16/8); PROD opretter sessioner.** Staging: `session.url` → betaling → `subscription_created` → provisionering → `✅ Firma oprettet`. Prod: `session.url` returneret, og `company`-valideringen bevist i begge retninger. **At prod fik en session betyder, at prod-kontoen HAR en betalingsmetode** — ellers var svaret `601`. **Tilbage:** ét fuldt gennemløb i prod (venter på flere numre i puljen) · aflæsning af prod-kontoens plan-handle og prøveperiode · flowet kørt fra partnersitet.
