...
**I GANG — præcis én**
- Fundamentsporet, dag 1½ af 10 — opsætning + dokumentkonsolidering (O7). Dag 1 lukket 4/8-26
...
|---|---|---|---|---|---|
| D1 | Gendannelsesøvelsen dækker nu **også Storage** — filer og metadata gendannes sammen, og lead-billeder er bevist genskabelige | M | M | **Mitigeret** | **Bestået 4/8-26** til isoleret midlertidigt projekt: rækketal identiske, 76 filer i 3 buckets (157,9 MB), billed- og lydfil åbnet fra målet. Procedure og øvelseslog i runbogen. Gentages hvert kvartal sammen med D4 |
...
| D21 | **`C:\Windows\System32` mangler i maskinens PATH** — `more` og andre indbyggede værktøjer kan ikke findes. Ramte `psql`-output 4/8-26; kan ramme andre værktøjer uforudsigeligt | L | L | Åben | Tilføj til PATH. Ikke hastende, men det vil koste tid igen på et ubelejligt tidspunkt |
...
| S12 | **Bucket `potentielle kunder` er offentlig** — alle med URL'en kan læse indholdet uden login. Tom pr. 4/8-26, så vinduet er lukket lige nu. `greetings` er også offentlig, men bevidst: Twilio skal kunne hente lyden | M | M | Åben | Afgør om bucket'en skal bruges. Skal den, så gør den privat og skriv en politik, før første fil lægges i |
...
| J4 | Ingen slettepolitik — hverken på papir eller i kode. **Fund 4/8-26: 1 lead, men 36 lead-billeder** — 35 forældreløse billeder af tredjeparter, som ingen bruger kan læse (politikken slår op på lead-id) og som ingen sletning fjerner | H | M | Åben | Frister pr. datakategori. Husk bogføringslovens 5 år og backup-rotation. Sletning af et lead skal også fjerne dets filer i Storage |
...
|---|---|---|---|---|
| **1** ✅ | Sandheden om produktion (lukket 4/8-26) | D1 restore · D4 + D10 rollback · **D16 sammenligning Railway ↔ `.env.prod`** | **Nej — gør det selv.** Pointen er, at Ann kan gøre det kl. 22. D16-scriptet rører prodvariabler, som Claude Code ikke må | Gendannelse gentaget **inkl. Storage**, manifest findes, én lydfil og ét billede åbnet · en deploy rullet tilbage · afvigelser mellem Railway og `.env.prod` fundet og ryddet |
...
| 2026-08-04 | **D1 lukket som mitigeret: gendannelsesøvelsen gentaget inkl. Storage.** Fjerde `pg_dump` (storage-metadata), `dump-storage.js` og `restore-storage.js` skrevet og kørt. 76 filer i 3 buckets gendannet til isoleret midlertidigt projekt; rækketal identiske; billed- og lydfil åbnet fra målet. Runbogens gendannelsesafsnit omskrevet med Storage på rigtig plads, bucket-politikker noteret, `TILLAEG-gendannelse-storage.md` kan slettes |
| 2026-08-04 | **Beslutning: gendannelse sker til et nyoprettet, isoleret projekt — ikke staging.** Mulighed C fra 2/8 er dermed afløst: den løste instrument-problemet for rollback-øvelsen, som nu er gennemført, og den løste hverken tømningen eller filtællingen. Begrundelse i runbogen |
| 2026-08-04 | **Prod-projektet omdøbt fra `anneann1904@gmail.com's Project` til `ditdigitalekontor-prod`** og identificeret på ref krydstjekket mod Railway. Organisationen `AnneAnn` bekræftet på Pro; alle tre projekter ligger i den. Nano-instanserne er arv fra Free-opgraderingen, ikke en manglende plan. To organisationer hedder begge `AnneAnn` — bør omdøbes |
| 2026-08-04 | **S12 og D21 oprettet, J4 udvidet.** Øvelsen afdækkede 35 forældreløse lead-billeder og en offentlig, ubrugt bucket. Ø1-note: tre aktive projekter betyder compute ud over de $10 i credits |
