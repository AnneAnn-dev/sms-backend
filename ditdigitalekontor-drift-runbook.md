...
✅ **BESTÅET 4/8-26** — prod gendannet til et isoleret, midlertidigt Supabase-projekt, **inklusive Storage**. Verificeret: identiske rækketal i `public`, kendt firma slået op, 76 af 76 filer i tre buckets, og én lead-billedfil + én greeting-lydfil hentet ned fra målet, åbnet og afspillet.

*Princippet står stadig: en backup du aldrig har gendannet, er et håb.*

**Kadence:** hvert kvartal, sammen med rollback-øvelsen (D4).

---

### Hvad en gendannelse består af — og hvorfor rækkefølgen er, som den er

Fem trin, hvor hvert trin forudsætter det foregående:

| Trin | Fil / handling | Hvad den opretter | Hvorfor den skal komme her |
|---|---|---|---|
| 1 | `schema.sql` | Tabeller, indekser, triggere, views, RLS-politikker på `public` | `data.sql` har intet at putte rækker i uden den |
| 2 | `auth-users.sql` | Brugerne i `auth.users` og `auth.identities` | `firm_users` og `storage.objects.owner` peger på dem med fremmednøgler |
| 3 | `data.sql` | Rækkerne i `public` | — |
| 4 | `storage-metadata.sql` | **Bucketsene** og de 76 objektrækker | En bucket *er* en række i `storage.buckets` — der findes ikke andet. Filerne kan ikke uploades til en bucket, der ikke findes |
| 5 | `restore-storage.js` | Selve filerne i S3 | Rækkerne findes allerede fra trin 4; scriptet bruger derfor `upsert` |

**De to huller, som `--schema=public` efterlader:** auth-brugerne ligger i `auth`-skemaet, og Storage ligger dels i `storage`-skemaet (metadata), dels helt uden for Postgres (filerne). Begge dumpes for sig. Gendannes filerne uden metadata-rækkerne eller omvendt, ser alt rigtigt ud i dashboardet, men appen kan ikke finde noget.

**Prioritering under en ægte katastrofe:** lead-billeder først — de er uploadet af håndværkerens kunder og kan ikke genskabes. Greeting-lyd kan genrenderes fra ElevenLabs med regenereringsscriptet.

---

### 1. Vælg et mål — og gør det tomt fra fødslen

**Regel: gendannelse sker til et isoleret, nyoprettet Supabase-projekt. Ikke til staging.**

Begrundelsen er ikke kun ægte kundedata. Beståelseskriteriet er et tal — antal filer pr. bucket — og det tal er kun sandt, hvis målet startede på nul. Staging har buckets med indhold, som så skal tømmes først, og tømningen er selv en destruktiv operation, der bagefter skal verificeres. Et nyt projekt behøver ingen bevisførelse. Dertil kommer, at bucket- og tabelkollisioner forsvinder helt, og at en tastefejl i en ref koster en spildt upload frem for staging.

**Ved oprettelsen:**

- **Region: West EU (Ireland), `eu-west-1`** — samme som prod. ⚠️ **Verificér regionen bagefter** under Primary Database. "eu-west" findes i to udgaver, og `eu-west-2` er London, altså uden for EU efter Brexit. **Regionen kan ikke ændres efter oprettelsen.**
- **Skriv databasepasswordet selv** i feltet frem for at bruge "Generate a password". Så kender du det fra fødslen. Gem det straks.
- Navn med dato: `ddk-gendannelsestest-ÅÅÅÅMMDD`.
- Compute bliver **Micro** — Nano kan ikke vælges på en betalt plan. Micro koster compute pr. time, så projektet skal slettes samme dag.
- Skriv "slet projektet" på dagens liste **med det samme**.

### 2. Tag snapshottet — fire `pg_dump`-kommandoer

Via **session-pooleren**. Den direkte forbindelse (`db.<ref>.supabase.co`) er IPv6-only og kan ikke nås fra vores netværk; transaction-pooleren (port 6543) knækker `pg_dump`.

```powershell
cd C:\Users\Bruger\ddk-gendannelse-ÅÅÅÅ-MM-DD
$env:PAGER = ''
...
**Arbejd uden for repoet.** Dumpene indeholder persondata og må aldrig committes. En mappe uden for `sms-backend` fjerner hele risikoen — bedre end at huske en `.gitignore`-linje.

**Verificér `storage-metadata.sql`, før du går videre:**

```powershell
Select-String -Path storage-metadata.sql -Pattern "^COPY"
```

Der skal stå to `COPY`-linjer, **`storage.buckets` før `storage.objects`**. Rækkefølgen betyder noget ved gendannelsen, fordi objekter peger på buckets. `path_tokens` skal **ikke** stå i kolonnelisten — den er en genereret kolonne, som `pg_dump` med rette springer over, og målet beregner den selv.

Tæl rækkerne (linjetallene 26 og 37 kommer fra udskriften ovenfor):

```powershell
$linjer = Get-Content storage-metadata.sql
$slut = @(0..($linjer.Count - 1) | Where-Object { $linjer[$_] -eq '\.' })
"terminatorer fundet: " + $slut.Count    # skal være 2
"buckets:  " + ($slut[0] - 26)
"objekter: " + ($slut[1] - 37)
```

`\.` er PostgreSQLs markør for enden af en `COPY`-blok. Er `terminatorer fundet` ikke 2, er filen ikke, som vi tror — kig på den frem for at regne på den.

### 3. Hent filerne ud — `dump-storage.js`

Ligger i repo-roden. Læser aldrig `.env`; nøglerne sættes i sessionen og hentes fra dashboardet (D16: `.env.prod` har kendte fejl).

```powershell
cd C:\Users\Bruger\sms-backend
$env:DDK_SOURCE_URL = 'https://<PROD_REF>.supabase.co'
$env:DDK_SOURCE_SERVICE_KEY = 'sb_secret_...'      # service_role/secret — publishable rækker ikke
node dump-storage.js --ref <PROD_REF> --out "C:\Users\Bruger\ddk-gendannelse-ÅÅÅÅ-MM-DD\storage-backup" --dry-run
node dump-storage.js --ref <PROD_REF> --out "C:\Users\Bruger\ddk-gendannelse-ÅÅÅÅ-MM-DD\storage-backup"
```

`--dry-run` lister uden at hente og giver antal og samlet størrelse først. Scriptet nægter at køre uden `--ref` og `--out`, og `--ref` skal stemme med ref'en i URL'en. **Læs ekko-blokken, før du går videre** — der står, hvilket projekt den taler med.

Resultatet er `storage-backup/<bucket>/<sti>` plus **`storage-manifest.json`**. Manifestet er beviset: ingen manifest = ingen bestået øvelse.

### 4. Gendan ind i målet — fire `psql`-kørsler i rækkefølge

Hent forbindelsesstrengen fra **Connect → Session pooler** i målprojektet. Konstruér den ikke selv; værtsnavnet kan afvige.
...
psql "postgresql://postgres.<MÅL_REF>@aws-0-eu-west-1.pooler.supabase.com:5432/postgres" -f storage-metadata.sql
```

Én ad gangen, med udskriften læst mellem hver. **Test altid forbindelsen først** med et lille kald frem for at kaste en fil på 1.700 linjer efter den:

```powershell
psql "postgresql://postgres.<MÅL_REF>@aws-0-eu-west-1.pooler.supabase.com:5432/postgres" -P pager=off -c "select current_user, current_database();"
```

Fejler et `select`, ved du præcis hvad der er galt. Fejler `schema.sql` halvvejs, skal du bagefter finde ud af, hvor langt den nåede.

**Opret ikke bucketsene i dashboardet først.** `storage-metadata.sql` opretter dem, fordi en bucket *er* en række. Gør du det manuelt, fejler indsættelsen på en nøglekonflikt.

### 5. Læg filerne på plads — `restore-storage.js`

```powershell
cd C:\Users\Bruger\sms-backend
$env:DDK_TARGET_URL = 'https://<MÅL_REF>.supabase.co'
$env:DDK_TARGET_SERVICE_KEY = 'sb_secret_...'      # MÅLETS nøgle, ikke prods
node restore-storage.js --ref <MÅL_REF> --in "C:\Users\Bruger\ddk-gendannelse-ÅÅÅÅ-MM-DD\storage-backup" --dry-run
node restore-storage.js --ref <MÅL_REF> --in "C:\Users\Bruger\ddk-gendannelse-ÅÅÅÅ-MM-DD\storage-backup"
```

Tre spærrer: den nægter at skrive til kilden (manifestet husker, hvor dumpet kom fra), prods ref står i `FORBUDTE_REFS` i koden, og alle filer verificeres på disken, før den første upload sker. Scriptet kan køres flere gange — det bruger `upsert`.

Trin 4 i scriptet tæller selv i målet og sammenligner med manifestet. **Det er beståelseskriteriet, målt automatisk.**

### 6. Ryd op

- **Slet målprojektet samme dag.** Micro koster compute pr. time, og et glemt projekt er en udgift, hvis årsag er umulig at rekonstruere om tre måneder.
- **Slet dumpene**, eller flyt dem til sikker off-site-placering. Behold gerne kun `storage-manifest.json` — det er beviset og indeholder stier og størrelser, ikke indhold.
- `Clear-History` — passwords havner i PowerShell-historikken.

---

### Beståelseskriteriet

Øvelsen er først bestået, når **alle** punkter er afkrydset:

- [ ] Rækketallene i `public` stemmer mellem kilde og mål
- [ ] Et kendt firma kan slås op i målet
- [ ] Antal filer pr. bucket stemmer med `storage-manifest.json`
- [ ] **Én lead-billedfil og én greeting-lydfil hentet ned fra målet og åbnet** — de skal kunne vises og afspilles, ikke bare findes
- [ ] Dato og filantal skrevet i øvelsesloggen nedenfor

Struktur- og rækketalstjek, kørt i **begge** projekters SQL-editor:
...
⚠️ **Antal filer beviser ikke, at filerne findes.** Både `storage-metadata.sql` og Storage-API'ets `list()` læser fra `storage.objects` — samme kilde, to veje. En metadata-række, hvis fil er væk fra S3, ville blive talt med begge steder. **Den ægte verifikation er selve downloadet:** `dump-storage.js` henter bytes ned og sammenligner størrelsen med den, metadata påstår. Derfor er gennemløb 2 beviset, ikke listen.

---

### Bucket-politikker — følger IKKE med en gendannelse

Rækken i `storage.buckets` bærer navnet og `public`-flaget. Adgangsreglerne er RLS-politikker på `storage.objects` og dumpes ikke. De skal oprettes på ny i målet, hvis adgang skal testes gennem brugerkontekst. Til selve øvelsen mærkes det ikke, fordi scripterne bruger `service_role`, som går uden om RLS — men under en ægte katastrofe er de nødvendige.

**Buckets i prod pr. 4/8-26:**

| Bucket | Offentlig | Politikker | Indhold |
|---|---|---|---|
| `greetings` | **Ja** | 0 | Renderet velkomstlyd. Offentlig, fordi Twilio skal kunne hente den |
| `lead-images` | Nej | 1 | Billeder uploadet af håndværkerens kunder. **Uerstattelige** |
| `potentielle kunder` | **Ja** | 0 | Tom pr. 4/8-26. ⚠️ Offentlig — bør vurderes, se registret |

⚠️ Bucket-navnet `potentielle kunder` indeholder et **mellemrum**. Det skal håndteres i stier og URL'er.

**Den gældende politik — gendan ordret:**

```sql
CREATE POLICY lead_images_storage_read ON storage.objects FOR SELECT TO authenticated
USING (((bucket_id = 'lead-images'::text) AND ((storage.foldername(name))[1] IN (
  SELECT (l.id)::text AS id
  FROM (leads l JOIN calls c ON ((c.id = l.call_id)))
  WHERE (c.firm_id IN (
    SELECT firm_users.firm_id FROM firm_users WHERE (firm_users.user_id = auth.uid())))))));
```

Læst i klartekst: en indlogget bruger må læse et billede, hvis mappenavnet svarer til et lead-id, hvis opkald hører til et firma, brugeren er medlem af. Mappenavnet **er** lead-id'et — det er bærende for hele adgangsmodellen.

Hent den frem igen sådan her (dashboardets celle klipper lange politikker af):

```powershell
psql "postgresql://postgres.<PROD_REF>@aws-0-eu-west-1.pooler.supabase.com:5432/postgres" -P pager=off -A -t -o politik.txt -c "select 'CREATE POLICY ' || quote_ident(policyname) || ' ON storage.objects FOR ' || cmd || ' TO ' || array_to_string(roles, ', ') || ' USING (' || qual || ');' from pg_policies where schemaname = 'storage' and tablename = 'objects';"
```
...
| Meddelelse | Hvorfor |
|---|---|
| `ERROR: permission denied to change default privileges` | Supabase-projekter har dem sat fra fabrikken. Nogle `ALTER DEFAULT PRIVILEGES`-linjer lykkes, andre ikke — det afhænger af, hvilken rolle de tilhører |
| `ERROR: schema "public" already exists` | Nye projekter fødes med `public`. Alt derefter lander i det eksisterende skema |
| `'more' is not recognized` | `psql` kalder sin sideviser, når der er output at vise. **At se den betyder, at forbindelsen lykkedes.** Sæt `$env:PAGER = ''` eller brug `-P pager=off` |
| `Assertion failed: !(handle->flags & UV_HANDLE_CLOSING)` | libuv på Windows ved `process.exit()` efter netværkskald. Kommer **efter** scriptets sidste linje — tallene på skærmen er sande |

---

### Fejlfinding: forbindelsen

**Poolerens fejltekst siger altid `for user "postgres"`**, uanset hvad du sendte. Den splitter `postgres.<ref>` op i tenant og rolle og videresender kun rollen. Fejlteksten kan derfor **ikke** bruges til at afgøre, om ref-suffikset kom med. Samme grund til at `select current_user` svarer `postgres`.

**Teknikken der afgør det:** fremprovokér en fejl, du kender betydningen af, og sammenlign.

```powershell
psql "postgresql://postgres.detteerikkeenref@aws-0-eu-west-1.pooler.supabase.com:5432/postgres" -c "select 1;"
```

| Svar | Betydning |
|---|---|
| `(ENOTFOUND) tenant/user ... not found` | Pooleren kender ikke projektet: forkert ref, forkert regionsvært, eller projektet er ikke færdigt med at rejse sig |
| `password authentication failed` | Forbindelsen er nået frem. Kun passwordet mangler |

Er ref og vært rigtige, og passwordet stadig afvises på et **midlertidigt** projekt: brug ikke tid på årsagen. Slet projektet og opret et nyt med et password, du selv skriver. Et engangsproblem på et engangsprojekt er ikke værd at forstå til bunds — på prod ville det modsatte gælde.

---

### Regler, øvelsen 4/8-26 kostede tid at lære

1. **Dashboardet er facit — ikke en konstrueret forbindelsesstreng.** Vært, region og brugernavnsform skal hentes fra Connect → Session pooler for det konkrete projekt. Prods streng gælder ikke automatisk for et nyt projekt.
2. **Et projektnavn er ikke et bevis på, hvad projektet er.** Prod hed `anneann1904@gmail.com's Project` indtil 4/8-26. Identificér altid på **ref**, krydstjekket mod `SUPABASE_URL` i Railway.
3. **Læs projektnavnet i brødkrummen før enhver destruktiv handling** i dashboardet — password-reset, sletning, alt under Settings — og tag et skærmbillede. Det er den eneste bekræftelse, der gives, og den efterlader ellers ingen spor.
4. **Regionen skal verificeres efter oprettelsen.** `eu-west-1` er Irland, `eu-west-2` er London. Kan ikke ændres bagefter.
5. **`$env:PGPASSWORD` og `$env:PAGER` sættes i samme vindue og overlever hele sessionen.** Skiftes der mellem prod og mål, er variablen den eneste ting, der afgør hvilket projekt der skrives til — sammen med strengen. Læs ref'en i kommandoen, før du trykker.

---

### Øvelseslog

| Dato | Mål | Omfang | Resultat |
|---|---|---|---|
| 3/7-26 | staging | `public` + auth-brugere | ✅ Bestået. Storage ikke dækket |
| 4/8-26 | `ddk-gendannelsestest3-20260804` (isoleret, slettet samme dag) | `public` + auth + **Storage** | ✅ Bestået. `firms 3 · leads 1 · calls 12 · auth_users 6` identisk i kilde og mål. **76 filer i 3 buckets, 157,9 MB** — `greetings` 40, `lead-images` 36, `potentielle kunder` 0. Billedfil og lydfil åbnet fra målet |

**To målinger på række gør det synligt, hvis noget er holdt op med at blive gemt.** Skriv derfor altid filantal pr. bucket ind, ikke bare "bestået".

**Fund 4/8-26, som øvelsen afdækkede:** `leads 1` men `lead-images 36` — 35 billeder uden tilhørende lead. Under den gældende politik kan ingen bruger læse dem, da opslaget sker på lead-id. De er altså utilgængelige og gemt alligevel. Hører under J4, slettepolitik.
